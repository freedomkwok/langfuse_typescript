...
export class StorageServiceFactory {
  ...
  public static getInstance(params: {
    ...
    if (params.useOracleCloudStorage !== undefined
      ? params.useOracleCloudStorage
        : env.LANGFUSE_USE_ORACLE_CLOUD_STORAGE === "true"
    ) {
        return new OracleStorageService(params)
    }
...
class OracleStorageService implements StorageService {
  ...
  private async ensureInitialized() {
    if (!this.authProvider) {
      this.authProvider = await new common.InstancePrincipalsAuthenticationDetailsProviderBuilder().build();
      ...
      const namespaceResponse = await this.client.getNamespace(getNamespaceRequest);
  ...
  public async uploadFile({
    ...
    try {
      ...
      if (typeof data === "string") {
        putObjectRequest.contentLength = Buffer.byteLength(data);
      } else if (Buffer.isBuffer(data)) {
        putObjectRequest.contentLength = data.length;
      }
  ...
  public parseOciUrl(url: string) {
    const u = new URL(url);
    if (u.protocol !== "oci:") {
      throw new Error("Not an OCI URL");
    }

    const bucketName = u.hostname;
  ...
  public async uploadJson(
    ...
    await this.ensureInitialized();
    try {
      const { bucketName, objectName } = this.parseOciUrl(path);
      const content = JSON.stringify(body);
      const putObjectRequest: objectstorage.requests.PutObjectRequest = {
        namespaceName: this.namespace,
  ...
  toNodeReadable(value: Readable | ReadableStream): Readable {
    if (value instanceof Readable) return value;
    return Readable.fromWeb(value as any);
  }

  async streamToString(value: Readable | ReadableStream): Promise<string> {
    const stream = this.toNodeReadable(value);
    let result = "";
  ...
  public async download(path: string): Promise<string> {
    ...
    try {
      ...
      return await this.streamToString(getObjectResponse.value);
    } catch (err) {
      logger.error(
        `Failed to download file from oci cloud ${path}`,
        err,
      );
      handleStorageError(err, "download file from Oracle Storage");
    }
  }
  ...
  public async listFiles(
    ...
    try {
      ...
      const resp = await this.client!.listObjects(listObjectsRequest);
      const files = resp.listObjects.objects ?? [];

      return files.map((file) => ({
        file: file.name,
        createdAt: new Date(file.timeCreated ?? new Date()),
      }));
    } catch (err) {
      logger.error(
  ...
  public async getSignedUrl(
    ...
    try {
      ...
      const resp = await this.client!.createPreauthenticatedRequest(createParRequest);

      const par = resp.preauthenticatedRequest;
      console.log(par.fullPath, par.accessUri)
      let url = par.accessUri
      if (asAttachment) {
      ...
      handleStorageError(err, "generate signed URL for Oracle Cloud Storage");
    }
  }


  public async getSignedUploadUrl(params: {
    path: string;          // objectName in OCI
    ttlSeconds: number;
    sha256Hash: string;    // not enforced by PAR (optional validate later)
    contentType: string;   // not enforced by PAR (optional validate later)
    contentLength: number; // not enforced by PAR (optional validate later)
  }): Promise<string> {
    await this.ensureInitialized();
    ...
    try {
      ...
      const resp = await this.client!.createPreauthenticatedRequest(req);
      const par = resp.preauthenticatedRequest;

      console.log(par.fullPath, par.accessUri)
      const url = par.accessUri
      return url;
    } catch (err) {
      logger.error(
        `Failed to generate signed upload URL for OCI Object Storage ${params.path}`,
        err,
      );
      handleStorageError(err, "generate signed upload URL for OCI Object Storage");
      throw err;
    }
  }
  ...
  public async deleteFiles(paths: string[]): Promise<void> {
    ...
    try {
      await Promise.all(
        paths.map(async (path) => {
          ...
          try {
            ...
            await this.client!.deleteObject(req);
          } catch (err: any) {
            // Ignore "not found" (equivalent to ignoreNotFound: true)
      ...
      logger.error(`Failed to delete files from OCI Object Storage`, err);
